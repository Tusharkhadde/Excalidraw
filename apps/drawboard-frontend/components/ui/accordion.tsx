...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between gap-4 py-5 text-left text-[15px] font-medium transition-all hover:text-primary [&[data-state=open]>svg]:rotate-45 [&[data-state=open]]:text-primary",
        className,
      )}
      {...props}
    >
